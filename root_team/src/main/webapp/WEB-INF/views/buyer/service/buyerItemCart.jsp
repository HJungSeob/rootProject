...
<head>
    ...
    $(function() {
        ...
        $('.m_buyerItemCart_itemsCount').on('input', function() {       	
            ...
            $(this).closest('.m_buyerItemCart_items').find('.m_buyerItemCart_itemsPrice').text("₩"+totalPrice.toLocaleString());
        ...
        function updateTotalPrice() {
            ...
            $('.m_buyerItemCart_items').each(function() {
                var price = parseInt($(this).find('.m_buyerItemCart_itemsPrice').text().replace(/[^\d.]/g, ""));
                totalCartPrice += price;
            });
            $('.itemTotalPrice').text("₩" + totalCartPrice.toLocaleString());
            $('.allTotalPrice').text("₩" + (totalCartPrice + 3000).toLocaleString());
            $('.m_buyerItemCart_bagMainTotalPrice').text("장바구니 총액: " + $('.itemTotalPrice').text());
        } 
    });
    ...
    $(function() {
    	...
    	$(document).on("click", "#deleteBtn", function() {
            ...
            var i_option = $(this).closest(".m_buyerItemCart_items").find(".m_buyerItemCart_itemsTitle span").text();
            $.ajax({
                type: "POST",
        ...
        <section>
            <div class="m_buyerItemCart_bagRegion">
                <form action="">
                    ...
                    <label>
                        <input type="submit" class="m_buyerItemCart_bagSubmit">
                        <div class="m_buyerItemCart_bagSubmitBtn"><a href="${pageContext.request.contextPath}/cart/order.do">결제</a></div>
                    </label>            
                ...
                <c:choose>
                                ...
                                <c:if test="${not empty cart}">
                                    <div>                                   	
				                        ...
				                        <input type="hidden" class="itemPrice" value="${cart.i_price}">			                        
				                            <img src="${pageContext.request.contextPath}/resources/uploads/${cart.i_img}" alt=""
				                                class="m_buyerItemCart_itemsImg">
				                            <div>
				                                <div class="m_buyerItemCart_itemsTitle">
					                                <a href="${pageContext.request.contextPath}/item/view.do?i_idx=${cart.i_idx}">
					                                	${cart.i_name}
					                                </a><br>
				                            ...
				                            <div>
				                                ...
				                                <fmt:formatNumber value="${cart.i_price}"
                                                    type="currency" currencySymbol="₩" />				                                
				                                </div>
				                                <div class="m_buyerItemCart_itemsCancel"><button id="deleteBtn">삭제</button></div>
				                                <div class="m_buyerItemCart_itemsCancel"><button id="saveBtn">나중을 위해 저장</button></div>
				                            </div>
				                        </div>
				                        <div class="m_buyerItemCart_hrTag"></div>
				                    </div>
                    ...
                    <div>
                        ...
                        <div class="m_buyerItemCart_TotalSubmitBtn">결제</div>
                    </div>
                    <div class="m_buyerItemCart_hrTag"></div>
