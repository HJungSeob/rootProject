...
<head>
	...
	<script>
		function sendEmailVerification() {
			...
			$.ajax({
				type: 'post',
				url: '${pageContext.request.contextPath}/seller/joinEmailProcess.do',
				data: {
					s_email: s_email
				},
				dataType: "text",
		...
		$(function () {
			$("#confirm_email_btn").on("click", function () {
				...
				if (inputCode == code) {
					...
					$.ajax({
						type: 'post',
						url: '${pageContext.request.contextPath}/seller/sellerVerifyEmailProcess.do',
						data: {
							s_email: s_email
						},
						success: function (data) {
							if (data == 1) {
								...
								window.location.href = "${pageContext.request.contextPath}/seller/sellerLogin.do";
							}
						},
						error: function () {
							console.log("실패");
