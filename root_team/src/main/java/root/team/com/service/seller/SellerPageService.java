...
@Service("sPage")
public class SellerPageService implements SellerService {

	// 페이지 네비게이션을 위한 9개의 변수에 대한 초기화
	@Override
	public PageNav setPageNav(PageNav pageNav, int pageNum, int pageBlock) {
		int totalRows = pageNav.getTotalRows(); // 1. 총 게시물 수: totalRows
		int rows_page = 6; // 2. 한 페이지에 보여줄 게시물 수: rows_page
		int pages_pageBlock = 5; // 3. 한 페이지 블록에 보여줄 페이지 수: pages_pageBlock
		...
		pageNav.setPageNum(pNum); // 4. 현재 페이지 번호: pageNum
		...
		int last_pageBlock = (int) Math.ceil((double) total_pageNum / pages_pageBlock); // 9. 마지막 페이지블록번호 :
																						// last_pageBlock
